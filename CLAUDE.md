...
- Rendering backend: entry scripts set `MUJOCO_GL`/`PYOPENGL_PLATFORM` to `osmesa`, or to `egl` only when the hostname is `bishop`.
...
- Resume: a config named `<orig>_resume.py` with `trainer_dict['do_train_resume']=True` and `path_resume` pointing to a path that contains `<orig>` in its name.
- Config filename conventions: `antM`/`pntM`/`humM`/`antSoc`/`antMexpl` = env, `Gi/Lg/Me/Ar` = size, `o2d`/`o15d`/`o29d` = planned state dims (set via `dataset_config['obs_select_dim']`), `g*d` in invdyn configs = goal dims, `h160`/`ovlp56` = segment horizon / overlap length, `DiT...` = DiT backbone instead of the UNet.
...
- **Evaluation** (`ogb_task/ogb_maze_v1/plan_ogb_stgl_sml.py` → `OgB_Stgl_Sml_MazeEnvPlanner_V1` in `ogb_stgl_sml_planner_v1.py`): **per-environment eval hyper-parameters are hard-coded in the `__main__` block of `plan_ogb_stgl_sml.py`**, in a big `if dataset ...` chain, not in configs. That chain sets `ev_n_comp`, the sampling type, the replanning config (`repl_ada_dist_cfg`), `inv_epoch`, DDIM settings, and so on. The planner loads eval problems (100 start/goal pairs, 5 tasks × 20) from `data/ogb_maze/ev_probs/*.hdf5`. It rolls out in the env and writes `.mp4` and `.json` results.
- **Env/path registries**: `diffuser/utils/ogb_utils/ogb_serial.py` maps env name → eval-problem file (`get_ogb_maze_ev_probs_fname`), dataset normalizer loading, and env name + goal dim → **hard-coded inverse-dynamics checkpoint dir under `logs/`** (`ogb_get_inv_model_path`). Update it when adding an env or retraining an inv-dyn model under a new name.
